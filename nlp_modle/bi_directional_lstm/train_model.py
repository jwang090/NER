...
def train_crf_model(train_data, test_data, dicts, seq, save_path, re_train=False):
    file=open(params.fileresult, encoding='utf-8', mode='w')
    graph = tf.Graph()
    corpus_dict = dicts['corpus']
    pos_dict = dicts['pos']
    label_dict = dicts['label']
    conf = Config()
    conf.set_batch_size(50)
    ...
    with graph.as_default():
        ...
        t_model.get_crf_train_attention_graph(initializer)

        test_model = BidirectionalCRF(conf)
        test_model.get_crf_test_attention_graph()
    ...
    with sv.managed_session() as session:
        ...
        for i in range(epoch):
            ...
            for step, (p, r, f) in enumerate(zip(precision, recall, f1)):
                ...
                file.write('label %d, precision %f, recall %f; F1 %f \n' % (step, p, r, f))
            print('epoch %d, cost %f, mp: %f, mc: %f, f1: %f ' %
                  (i, loss / count, s_p, s_r,  s_f1))
            file.write('epoch %d, cost %f, mp: %f, mc: %f, f1: %f \n' %
                  (i, loss / count, s_p, s_r,  s_f1))
            file.flush()
            ...
            if i % 3 == 0 and i != 0:
                ...
                file.write('test epoch %d, cost %f, mp: %f, mc: %f, f1: %f detail:%s \n' %
                      (i, test_loss / test_count, test_s_p, test_s_r, test_s_f1, msg_test))
                file.flush()
        file.close()
